...
public class GsonUtilsTest
{
	...
	class GetPrettyJson
	{
		...
		{
			Assertions.assertNull(GsonUtils.getPrettyJson(null));
			Assertions.assertNull(GsonUtils.getPrettyJson(""));
		}
		...
		void getPrettyJson_with_non_json_string()
		{
			...
			String response = GsonUtils.getPrettyJson(nonJson);
		...
		{
			String response = GsonUtils.getPrettyJson(mUserDtoJson);
			log.info("response: {}", response);

			UserDto responseDto = GsonUtils.toObject(response, UserDto.class);
	...
	class ToJsonStringToObject
	{
		...
		{
			Assertions.assertNull(GsonUtils.toJsonString(null));
		}

		@Test
		void toObject_with_blank_json_and_null_type()
		{
			Assertions.assertNull(GsonUtils.toObject(null, null));
			Assertions.assertNull(GsonUtils.toObject("", null));
		}

		@Test
		void toJsonString_toObject()
		{
			String userDtoString = GsonUtils.toJsonString(mUserDto);
			log.info("userDtoString: {}", userDtoString);

			UserDto userDtoResult = GsonUtils.toObject(userDtoString, UserDto.class);
	...
	class ToObjectList
	{
		...
		{
			Assertions.assertNull(GsonUtils.toObjectList(null, null));
			Assertions.assertNull(GsonUtils.toObjectList("", null));
		}
		...
		void toObjectList_with_correct_data()
		{
			...
			String json = GsonUtils.toJsonString(userDtos);
			log.info("json: {}", json);

			List<UserDto> userDtosResult = GsonUtils.toObjectList(json, UserDto.class);
	...
	class Parse
	{
		...
		{
			Assertions.assertNull(GsonUtils.parse(null));
			Assertions.assertNull(GsonUtils.parse(""));
		}

		@Test
		void parse_when_correct_args_t1()
		{
			JsonObject jsonObject = GsonUtils.parse(mUserDtoJson);
	...
	class GetJsonArray
	{
		...
		{
			Assertions.assertNull(GsonUtils.getJsonArray(null, null));
			Assertions.assertNull(GsonUtils.getJsonArray("", ""));
			Assertions.assertNull(GsonUtils.getJsonArray("", null));
			Assertions.assertNull(GsonUtils.getJsonArray(null, ""));
		}
		...
		void getJsonArray_with_correct_data()
		{
			...
			String json = GsonUtils.toJsonString(hashMap);
			log.info("json: {}", json);

			JsonArray array = GsonUtils.getJsonArray(json, "userDtos");
	...
	class GetElementAs
	{
		...
		{
			Assertions.assertNull(GsonUtils.getElementAsType(null, null, null));
			Assertions.assertNull(GsonUtils.getElementAsType("", null, ""));
			Assertions.assertNull(GsonUtils.getElementAsType("", null, null));
			Assertions.assertNull(GsonUtils.getElementAsType(null, null, ""));
		}
	}
